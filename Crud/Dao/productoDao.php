...
class productoDao
{
    ...
    public function listarProductosApp($codigo_invitacion)
    {
        $conn = Conexion::getConexion();
        $sentencia = $conn->prepare("SELECT idtienda from tienda where codigo_invitacion = $codigo_invitacion;");
        $sentencia->execute();
    ...
    public function listarProductosAppPrecio($codigo_invitacion)
    {
        ...
        if ($valor === FALSE) {
            ...
            try {
                $query = $conn->prepare('SELECT p.Nombre,p.Marca,p.Precio_unit from producto p inner join inventario i on i.id_Inventario = p.inventario_id_Inventario where tienda_idtienda = ?;');
                $query->bindParam(1, $idtienda);
                $query->execute();                
                return $query->fetchAll();
            } catch (Exception $ex) {
