...
class ventaDao
{
    ...
    public function modificarVenta(ventaDto $ventaDto)
    {
        ...
        try {
            $query = $conn->prepare("UPDATE venta SET id_Venta=?, FechaVenta=?, HoraVenta=?, EstadoVenta=?, cliente_id_Cliente=? , tienda_idtienda=?, metododepago_ID_Met_pago=?, usuarios_documento=? ,usuarios_tienda_idtienda=? WHERE id_Venta=?");
