...
class FacturaDao
{
    ...
    public function listarTodosTienda($codigo_invitacion)
    {
        $conn = Conexion::getConexion();
        try {
            $listarFactura = "SELECT f.venta_id_Venta, p.Nombre, p.Marca, SUM(f.Cantidad) as Cantidad, f.Precio, f.Estado
            FROM factura f
            INNER JOIN venta v ON f.venta_id_venta = v.id_venta
            INNER JOIN tienda t ON t.idtienda = v.tienda_idtienda
            INNER JOIN producto p ON p.id_Producto = f.producto_id_Producto
            WHERE t.codigo_invitacion = ?
            GROUP BY f.venta_id_Venta, p.Nombre, p.Marca, f.Precio, f.Estado";
