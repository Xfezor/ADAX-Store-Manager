...
class UsuarioDao
{
    ...
    //Modificar usuario solo 6 parametros (nombre1, nombre2, apellido1, apellido2, tipo_doc, correo)
    public function modificarUsuarioApp (UsuarioDto $usuarioDto)
    {
        $cnn = Conexion::getConexion();
        $mensaje = "";
        $documento = $usuarioDto->getDocumento();
        ...
        try {
            $query = $cnn->prepare("UPDATE usuarios SET nombre1=?, nombre2=?, apellido1=?, apellido2=?, tipo_doc=?, correo=? WHERE documento=?");
            $query->bindParam(1, $nombre1);
            $query->bindParam(2, $nombre2);
            $query->bindParam(3, $apellido1);
            $query->bindParam(4, $apellido2);
            $query->bindParam(5, $tipodoc);
